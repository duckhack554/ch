...
def register_on_chat(port_address):
    ...
    def confirm_nickname(port_address):
        ...
        registration_data = f"Ваш ник {name.decode('utf-8')}? Введите да или нет."
    ...
    def append_to_list(name, port_address):
        ...
        if data.decode('utf-8') == 'да':
            get_pass(name)
        elif data.decode('utf-8') == 'нет':
...
while True:
    ...
    if address not in clients:
        ...
        text = Fore.RED +"Регистрация прошла успешно. Добро пожаловать в чат!"
        print(Style.RESET_ALL)
